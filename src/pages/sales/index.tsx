      ...
      <div className="grid grid-cols-12 gap-4 p-[24px] lg:px-[120px] lg:py-[40px]">
        <div className=" hidden lg:block col-span-12 lg:col-span-3">
          <div className="sticky top-25 border-0 lg:border-r lg:border-[#CBD5E1] pr-0 lg:pr-4">
            ...
            {
              locations.map((item)=>{
                return(<div className="flex flex-col"  key={`location-${item.name}`} >
                    <p className="text-[#0F172A] text-[18px] font-semibold mb-[8px]">{item.name}</p>
                    <div className="flex flex-col mt-[16px] gap-[8px]">
                        {
                          item.cities.map((city)=>{
                            return (
                              <div className={`rounded-[6px] ${selectedCity?.value===city.slug ? 'bg-[#f4f4f5]':''} hover:bg-[#f4f4f5] py-[8px] px-[16px] cursor-pointer`}  key={`city-${item.name}-${city.city}`} onClick={()=>handleCityOnChange(city.slug, city.city)}>
                                <p className="text-[#09090B] text-[14px] font-semibold ">{city.city}</p>
                              </div>
                            )
                          })
                        }
          ...
          {sales.length > 0 && (
            <div className="flex flex-col justify-between h-full">
              <div className="flex flex-col gap-[24px]">
                {sales.map((item) => (
                  <div key={`item-${item.name}`} className="border-0 border-b lg:border-[#CBD5E1]" id={item.name} >
                    <h6 className="text-[30px] font-bold mb-[24px]" >{item.name}</h6>
                    <div className="flex flex-col gap-[24px] mb-[24px]">
                      {
                        item.sales.map((sales)=>{
                            return(
                              <SalesCard sales={sales} key={`item-${item.name}-${sales.name}`}/>
                            )
                        })
                      }
